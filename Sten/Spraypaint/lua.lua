...
do
	...
	function section:addSlider(title, default, min, max, callback, static, floorAmount)
		...
		local callback = function(value)
			if callback then
				callback(value, function(...)
					self:updateSlider(floorAmount, slider, ...)
				end)
			end
		end
		
		self:updateSlider(floorAmount, slider, nil, value, min, max)
		...
		slider.MouseButton1Down:Connect(function(input)
			...
			while dragging do
				...
				value = self:updateSlider(floorAmount, slider, nil, nil, min, max, value)
		...
		textbox.FocusLost:Connect(function()
			if not tonumber(textbox.Text) then
				value = self:updateSlider(floorAmount, slider, nil, default or min, min, max)
				callback(value)
			end
		end)
		...
		textbox:GetPropertyChangedSignal("Text"):Connect(function()
			...
			elseif not allowed[text] then	
				value = self:updateSlider(floorAmount, slider, nil, tonumber(text) or value, min, max)
				callback(value)
			end
		end)
	...
	function section:updateSlider(floorAmount, slider, title, value, min, max, lvalue)
		slider = self:getModule(slider)
		
		function Floor(x)
			return math.floor((x * (10 ^ (floorAmount or 1))) + 0.5) / (10 ^ (floorAmount or 1)) 
		end
		...
		value = value or Floor(min + (max - min) * percent)
